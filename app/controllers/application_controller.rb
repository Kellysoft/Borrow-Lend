class ApplicationController < ActionController::Base
  ...
  def user_authorization_lender
    if Lender.where(email: session[:user_email]).first
      @user = Lender.find(params[:id])
      redirect_to "/lender/#{current_user.id}" if current_user != @user
    end
  end

  def user_authorization_borrower
    if Borrower.where(email: session[:user_email]).first
      @user = Borrower.find(params[:id])
      redirect_to "/borrower/#{current_user.id}" if current_user != @user
    end
  end
